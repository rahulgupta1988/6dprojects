...
public class GPTagPostAdapter extends RecyclerView.Adapter<GPTagPostAdapter.LikeHolder>  {

    List<TagPostModel> tagPostModels;
    Context _mContext;


    public GPTagPostAdapter(Context context) {
        _mContext=context;

        tagPostModels= GPTagPostManager.tagPostModels;

    }


    @Override
    public GPTagPostAdapter.LikeHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.likegiven_item, parent, false);
        return new GPTagPostAdapter.LikeHolder(itemView);
    }

    @Override
    public void onBindViewHolder(GPTagPostAdapter.LikeHolder holder, int position) {

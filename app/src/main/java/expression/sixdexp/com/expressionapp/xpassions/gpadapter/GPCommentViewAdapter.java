...
public class GPCommentViewAdapter extends RecyclerView.Adapter<GPCommentViewAdapter.CommentHolder> {

    List<Comments> commentses;
    Context _mContext;


    public GPCommentViewAdapter(Context context) {
        _mContext = context;

        commentses = GpPostCommentManager.commentses;

    }


    @Override
    public GPCommentViewAdapter.CommentHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.commentgiven_item, parent, false);
        return new GPCommentViewAdapter.CommentHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final GPCommentViewAdapter.CommentHolder holder, final int position) {

        Comments comments = commentses.get(position);

        String imageUrl = comments.getImageurl();
        String username = comments.getUserName();
        String designamtion_txt = comments.getDepartment();

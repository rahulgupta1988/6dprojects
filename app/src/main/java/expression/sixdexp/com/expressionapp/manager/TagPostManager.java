...
public class TagPostManager extends BaseManager {

    Context context;
    String responseString = "";
   public static List<TagPostModel> tagPostModels=new ArrayList<>();

    public TagPostManager(Context context) {
        this.context = context;

    }

    public String callTagedUser(String recognitionid) {

        try {
            String userid=new LoginManager(context).getUserInfo().get(0).getUserid();
            HashMap<String, String> entitymap = new HashMap<String, String>();
            Log.i("responstring", "getpostbytaguser serviceUrl-->" + Constant.BASEURL + "api/tagging/getpostbytaguser");

            entitymap.put("userid", userid);
            entitymap.put("postid", recognitionid);
            responseString = ServerCall.makeConnection(Constant.BASEURL, entitymap, "api/tagging/getpostbytaguser");
